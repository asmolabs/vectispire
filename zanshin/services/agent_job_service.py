...
2. **Who may report on a job.** The agent that holds the lease, and nobody else —
   ...
   editing the finding set a gate is evaluated against (ADR-002 §5).
...
# somebody's source tree, and the whole point of ADR-002 §5's opt-in is that
...
class AgentJobService:
    ...
    def claim_task(self, db, agent: Agent, secure_transport: bool = True) -> Optional[ScanTask]:
        ...
        again when it is ready, which is the flow control ADR-002 D3 relies on —
    ...
    def _audit_credential_delivery(self, agent: Agent, scan: Scan, repo) -> None:
        ...
        only answerable question is *which* keys it received and when. ADR-002 §5
    ...
    def check_contract(self, reported_version: Optional[str]) -> None:
        ...
        (ADR-002 §8.5). Failing here, with both numbers in the message, is the
