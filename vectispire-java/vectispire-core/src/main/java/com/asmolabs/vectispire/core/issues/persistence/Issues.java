...
public interface Issues
    ...
    @Query("""
            select new com.asmolabs.vectispire.core.issues.persistence.OpenIssueCount(i.repoId, count(i.id))
              from IssueEntity i
    ...
    @Query("""
            select new com.asmolabs.vectispire.core.issues.persistence.OpenIssueCount(i.containerId, count(i.id))
              from IssueEntity i
     ...
     *
     * <p>The purge before it removes the triage events, ticket links and findings through Spring Data's derived deletes, which load
