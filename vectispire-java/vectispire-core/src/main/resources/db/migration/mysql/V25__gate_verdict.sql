...
-- jour.
-- **Les clés étrangères en clauses de table, et V19 dit pourquoi.** MySQL analyse un
-- `references` écrit dans la définition de colonne puis le jette : ni contrainte, ni cascade. Le
-- correctif de V19 portait sur vingt-quatre colonnes qui se lisaient comme des clés et n'étaient
-- que des entiers ; écrire la forme en ligne ici rouvrirait la même faille sur une table neuve,
-- et sur le seul moteur où les tests d'intégration de SQLite ne la verraient pas.
alter table t_gate_verdict add constraint fk_gate_verdict_repo
    foreign key (repo_id) references t_repository(id) on delete cascade;
alter table t_gate_verdict add constraint fk_gate_verdict_container
