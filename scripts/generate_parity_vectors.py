...
* **L'empreinte d'un problème** (`build_fingerprint`). Une divergence d'un seul
  ...
  triage déjà prise.
* **Le verdict du gate** (`policy_gate.evaluate`) et **les trois formats
  d'export**. Une divergence fait échouer — ou passer — la compilation de
  quelqu'un, ou produit un document remis à un auditeur qui n'a pas le même
  contenu que celui de la version précédente.

La chaîne d'intégrité du journal d'audit **ne figure plus ici** : elle a été
reconstruite côté TypeScript sur une forme d'horodatage canonique, précisément
pour cesser de dépendre du format de `datetime.isoformat()`. Voir
`backend/src/domain/audit/audit-hash.ts`.
