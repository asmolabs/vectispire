...
|---|---|---|---|
| [0001](0001-pluggable-scan-layer.md) | The scan layer is pluggable behind `ScannerEngine` | 2026-07-28 | accepted |
...
reasoning still holds — that is why they are here — but two of them now name artifacts the
NestJS port did not carry over: 0001's contract test and its two non-Docker engines, and
0006's fetch script. Each carries a dated note after its text, outside the decision, rather
than an edit to the decision itself.
