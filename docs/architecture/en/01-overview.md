...
**Two artifacts, one API.** A Spring Boot backend and an Angular front end; the browser now talks to the same HTTP API that a CI pipeline or a remote agent talks to.
...
| Secrets | `gitleaks` & `betterleaks` | **cut off** | `secret` findings |
