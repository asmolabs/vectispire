...
| **Elevation of Privilege** | Tentative d'exploitation d'une faille du parser de l'analyseur pour s'échapper du conteneur et accéder au socket Docker hôte. | Prise de contrôle totale du serveur hôte Vectispire. | **Aucun conteneur ne monte le socket Docker — le plan de contrôle compris** ([ADR 0018](../../fr/decisions/0018-the-docker-socket-is-never-mounted.md)). Un `docker-socket-proxy` sur un réseau interne autorise `ping`, `version`, `info`, `containers`, `images` et `POST`, et refuse les secrets, les volumes, les réseaux et Swarm. Une instance d'`exec` peut être créée mais jamais démarrée (mesuré, voir l'ADR 0018). Les scanners tournent avec `cap_drop: ALL`, `no-new-privileges`, racine en lecture seule et source montée en `read-only`. **Résiduel, et nommé** : `POST /containers/create` accepte des `Binds`, donc une exécution de code *dans le plan de contrôle* atteint encore l'hôte. Seul un agent distant sépare les deux. |
...
| Catégorie STRIDE | Scénario de Menace / Vecteur d'Attaque | Impact Potentiel | Mesure de Contrôle & Mitigation Implémentée |
|---|---|---|---|
| **Spoofing** | Un agent non autorisé tente de se connecter au plan de contrôle pour récupérer des travaux de scan. | Exfiltration du code des projets ou falsification des rapports d'analyse. | Authentification obligatoire par jeton d'agent révoquable (`t_agent`) et protocole exclusif HTTP Long-Polling (`/api/v1/agent/jobs`). Les jetons porteurs refusés sont plafonnés par adresse et le plafond est audité. |
| **Tampering** | Une **clé API d'agent volée** sert à rendre un résultat vide, ce qui résout tout le backlog de la cible pour ce type. | Toutes les vulnérabilités ouvertes d'une cible disparaissent en silence des écrans, des exports et du verdict de gate, ne laissant qu'un scan qui a l'air d'avoir tourné. | **Attestation du résultat** ([`ResultAttestation`](../../../../vectispire-java/vectispire-common/src/main/java/com/asmolabs/vectispire/common/domain/crypto/ResultAttestation.java)) : un opérateur épingle une clé publique Ed25519 sur la ligne de l'agent, hors bande, et les résultats de cet agent doivent alors porter une signature couvrant l'identifiant du scan et le corps exact. **La clé n'est jamais annoncée par l'agent** — une signature vérifiée contre une clé que son signataire a publiée sur le même canal ne prouve que ce que le jeton porteur prouvait déjà. Les refus sont audités (`AGENT_RESULT_REFUSED`). **Opt-in par agent** : un agent sans clé épinglée se comporte comme avant, ce n'est donc pas un contrôle actif jusqu'au jour où l'on s'aperçoit qu'il ne l'est pas. |
| **Information Disclosure** | Un **proxy qui termine TLS** entre l'agent et le plan de contrôle retire de l'annonce la clé de scellement de l'agent, ou y met la sienne, pour recevoir une clé SSH ou un jeton HTTPS délégué qu'il peut lire. | L'identifiant de déploiement d'un dépôt lu par qui administre le proxy — la seule partie que le scellement existe pour exclure. | **Clé de scellement signée** ([`SealingKeyAttestation`](../../../../vectispire-java/vectispire-common/src/main/java/com/asmolabs/vectispire/common/domain/crypto/SealingKeyAttestation.java), [ADR 0031](../../fr/decisions/0031-a-sealing-key-is-believed-only-on-the-pinned-key.md)) : l'agent signe sa clé X25519 éphémère avec sa clé de signature des résultats épinglée, sur son identifiant et la génération de la paire ; le plan de contrôle n'accepte qu'une clé qui se vérifie, garde la plus récente, et ne laisse jamais une annonce non signée ou absente la remplacer ou l'effacer. **Aucune remise en clair** : un identifiant délégué part scellé pour la clé vérifiée ou ne part pas, et un agent sans clé de signature épinglée n'en reçoit aucun. Les refus sont audités (`AGENT_SEALING_KEY_REFUSED`) et envoyés au SIEM (`ZAN-SEC-020`). **Pourvu qu'une clé de signature soit épinglée** : qui détient la configuration de l'agent peut toujours annoncer une clé à lui, et un proxy peut toujours refuser le service. |
