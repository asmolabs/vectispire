...
[`EncryptionService`](../../vectispire-java/vectispire-core/src/main/java/com/asmolabs/vectispire/core/crypto/EncryptionService.java):
...
```bash
VECTISPIRE_PREVIOUS_ENCRYPTION_KEYS="<your-previous-key>" cd vectispire-java && ./gradlew :vectispire-core:bootRun
```
...
3. **Set `ENCRYPTION_KEY`** in Vectispire's environment *before* saving the new key —
...
If `VECTISPIRE_BOOTSTRAP_PASSWORD` was filled in a compose file, an environment file or a CI
