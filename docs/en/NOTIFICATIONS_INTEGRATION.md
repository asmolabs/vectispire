...
* **The SIEM export follows the same rule, over every protocol.** Its endpoint — a URL for the webhook, `host:port` for syslog over UDP, TCP or TLS — is refused on a private address unless `notification_allow_private_url` is on, and a syslog host goes through the same address classifier as a URL. A SIEM collector on an internal network needs that setting. Its authorization header is sent with the webhook only, encrypted at rest like the other credentials, and leaving the field empty when saving keeps the stored one. Events leave through the same outbox as notifications, after the change that caused them commits — see the [SIEM export](../../docs-site/integrations/siem.md) page and [decision 0025](../architecture/en/decisions/0025-siem-events-leave-through-the-outbox.md).
