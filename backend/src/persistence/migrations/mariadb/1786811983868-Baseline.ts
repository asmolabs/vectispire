...
export class Baseline1786811983868 implements MigrationInterface {
    name = 'Baseline1786811983868'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`t_api_key\` (\`id\` uuid NOT NULL, \`name\` varchar(255) NOT NULL, \`key_hash\` varchar(255) NOT NULL, \`prefix\` varchar(16) NULL, \`created_at\` datetime(6) NOT NULL, \`last_used_at\` datetime(6) NULL, \`scopes\` varchar(255) NOT NULL DEFAULT 'read,scan,export', \`target_kind\` varchar(20) NULL, \`target_id\` int NULL, \`expires_at\` datetime(6) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`t_agent\` (\`id\` uuid NOT NULL, \`name\` varchar(255) NOT NULL, \`description\` varchar(500) NULL, \`kind\` varchar(20) NOT NULL, \`labels\` varchar(255) NULL, \`credentials_mode\` varchar(20) NOT NULL, \`enabled\` tinyint NOT NULL, \`max_concurrent\` int NULL, \`api_key_id\` uuid NULL, \`hostname\` varchar(255) NULL, \`platform\` varchar(255) NULL, \`version\` varchar(50) NULL, \`scanner_engine\` varchar(50) NULL, \`sealing_public_key\` varchar(255) NULL, \`capabilities\` json NULL, \`contract_version\` varchar(20) NULL, \`last_seen_at\` datetime(6) NULL, \`created_at\` datetime(6) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`t_container\` (\`id\` int NOT NULL AUTO_INCREMENT, \`registry\` varchar(255) NULL, \`image_name\` varchar(255) NOT NULL, \`tag\` varchar(255) NOT NULL, \`required_agent_label\` varchar(255) NULL, \`scan_interval_minutes\` int NULL, \`scan_cron\` varchar(255) NULL, \`last_scheduled_scan_at\` datetime(6) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`t_ssh_key\` (\`id\` uuid NOT NULL, \`name\` varchar(255) NOT NULL, \`private_key\` text NOT NULL, \`public_key\` text NULL, \`created_at\` datetime(6) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`t_repository\` (\`id\` int NOT NULL AUTO_INCREMENT, \`url\` varchar(255) NOT NULL, \`branch\` varchar(255) NOT NULL, \`sub_path\` varchar(255) NULL, \`required_agent_label\` varchar(255) NULL, \`name\` varchar(255) NULL, \`scan_interval_minutes\` int NULL, \`scan_cron\` varchar(255) NULL, \`last_scheduled_scan_at\` datetime(6) NULL, \`ssh_key_id\` uuid NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`t_scan\` (\`id\` int NOT NULL AUTO_INCREMENT, \`branch\` varchar(255) NOT NULL, \`sub_path\` varchar(255) NULL, \`status\` varchar(255) NOT NULL, \`required_agent_label\` varchar(255) NULL, \`sbom\` json NULL, \`cves\` json NULL, \`summary\` json NULL, \`duration_ms\` bigint NULL, \`findings_count\` int NOT NULL DEFAULT '0', \`new_issues_count\` int NOT NULL DEFAULT '0', \`resolved_issues_count\` int NOT NULL DEFAULT '0', \`error\` text NULL, \`created_at\` datetime(6) NOT NULL, \`version\` varchar(255) NULL, \`project_type\` varchar(255) NULL, \`repo_id\` int NULL, \`container_id\` int NULL, \`claimed_by\` varchar(64) NULL, \`claimed_at\` datetime(6) NULL, \`lease_expires_at\` datetime(6) NULL, \`attempts\` int NOT NULL DEFAULT '0', INDEX \`idx_scan_file\` (\`status\`, \`required_agent_label\`, \`created_at\`, \`id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`t_audit_log\` (\`id\` uuid NOT NULL, \`description\` varchar(255) NOT NULL, \`operation_type\` varchar(255) NOT NULL, \`resource_id\` varchar(255) NOT NULL, \`timestamp\` datetime(6) NOT NULL, \`user_id\` varchar(255) NULL, \`ip_address\` varchar(64) NULL, \`user_agent\` varchar(255) NULL, \`previous_hash\` varchar(64) NULL, \`entry_hash\` varchar(64) NULL, INDEX \`idx_audit_log_ordre\` (\`timestamp\`, \`id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`t_login_attempt\` (\`id\` uuid NOT NULL, \`counter_key\` varchar(255) NOT NULL, \`occurred_at\` datetime(6) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`t_outbox_message\` (\`id\` uuid NOT NULL, \`message_type\` varchar(50) NOT NULL, \`payload\` json NOT NULL, \`status\` varchar(20) NOT NULL, \`attempts\` int NOT NULL, \`next_attempt_at\` datetime(6) NULL, \`last_error\` text NULL, \`created_at\` datetime(6) NOT NULL, \`sent_at\` datetime(6) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`t_processed_message\` (\`id\` int NOT NULL AUTO_INCREMENT, \`message_id\` varchar(64) NOT NULL, \`message_type\` varchar(50) NOT NULL, \`agent_id\` uuid NULL, \`processed_at\` datetime(6) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
